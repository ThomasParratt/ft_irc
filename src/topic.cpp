...
int	Server::topicCommand(Msg msg, int clientSocket, Client &client)
{
	...
	else
	{
		...
		this->channel_names[i].setChannelTopic(msg.trailing_msg, client);
		std::string topicMsg = ":" + client.getPrefix() + " TOPIC " + channel_names[i].getChannelName() + " :" + msg.trailing_msg + "\r\n";
