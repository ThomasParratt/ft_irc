...
int		Server::removeUser(std::string user, std::string channel, std::string message, int partOrKick)
{
	...
	for (int j = 0 ; j < sizeof(_channel_names[i].channel_users) ; j++)
	{
		if (_channel_names[i].channel_users[j].nickname == user)
		{
			int socket = getClientSocket(_channel_names[i].channel_users[j].nickname);
			_channel_names[i].channel_users.erase(_channel_names[i].channel_users.begin() + j);
